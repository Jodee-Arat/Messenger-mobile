...
interface ChatMembersSectionProps {
	members: FindChatByChatIdQuery['findChatByChatId']['members']
	roles: ChatRoleData[]
	isLoading: boolean
...
const ChatMembersSection: FC<ChatMembersSectionProps> = ({
	...
	const handleRemoveMember = (
		member: FindChatByChatIdQuery['findChatByChatId']['members'][0]
	) => {
		if (member.isCreator) return
		Alert.alert(
			t('removeMember') || 'Удалить участника',
			(t('removeMemberConfirm') || 'Удалить {name}?').replace(
				'{name}',
